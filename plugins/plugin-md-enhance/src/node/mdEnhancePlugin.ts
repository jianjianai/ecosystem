...
import type { MarkdownPluginOptions } from './options.js'
import {
  prepareConfigFile,
  prepareMathjaxStyle,
...
import { isInstalled, logger, PLUGIN_NAME } from './utils.js'

export const mdEnhancePlugin =
  (
    options: MarkdownPluginOptions = { gfm: true },
    legacy = true,
  ): PluginFunction =>
  (app) => {
    ...
    const getStatus = (
      key: keyof MarkdownPluginOptions,
      gfm = false,
      pkgs: string[] = [],
    ): boolean => {
      const enabled =
    ...
    return {
      ...
      extendsMarkdown: (md): void => {
        ...
        if (status.imgMark)
          md.use(imgMark, isPlainObject(options.imgMark) ? options.imgMark : {})
